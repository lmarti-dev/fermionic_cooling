...
def plot_single(edm: ExperimentDataManager, dirnames, labels):

    fig, ax = plt.subplots()
    markers = iter("oxsd")
    for label, dirname in zip(labels, dirnames):
        runs = os.listdir(dirname)
        ...
        ax.plot(target_betas, np.array(final_fids), label=label, marker=next(markers))
    ax.set_xlabel(r"Target $\beta$")
...
if __name__ == "__main__":
    ...
    dirnames, labels = evening_plot()

    edm = ExperimentDataManager(experiment_name="plot_comparison_thermalizing")
    plot_single(edm, dirnames, labels)
